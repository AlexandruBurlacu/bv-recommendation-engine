...
class TestLogicModule(unittest.TestCase):
    ...
    def test_reshape_transform_valid_data(self):
        data = [(-1, 'fear', "tok", 0), (-1, 'sadness', "tok", 1),
                ...
                (1, 'surprise', "tok", 4)]
        self.assertEqual(reshape_transform(data), {'anger': [], 'fear': [(-1, 0), (-1, 3)],
    ...
    def test_get_max_len_valid_data(self):
        ...
        self.assertEqual(get_max_len(tmls), 20)
    ...
    def test_fill_valid_data(self):
        ...
        self.assertEqual(fill(my_range, 8), [1, 0, 0, -1, 0, 1, -1, 0])
    
    def test_fill_empty_data(self):
        my_range = []
        self.assertEqual(fill(my_range, 8), [0, 0, 0, 0, 0, 0, 0, 0])

    def test_chunk_sum_valid_data(self):
        x = range(1000)
        self.assertEqual(chunk_sum(x), [x * 100 + 45 for x in range(100)])

    def test_chunk_sum_min_data(self):
        self.assertEqual(chunk_sum(range(100)), list(range(100)))

    def test_chunk_sum_invalid_data(self):
        self.assertRaises(chunk_sum(range(10)), AssertionError))
